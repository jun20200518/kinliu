(function () {
  const ws = new WebSocket("wss://zb.objkss.cc/socket/");
  ...
  const chartCandleOptions = {
    ...
    timeScale: {
      borderColor: 'rgba(207, 211, 232, 0.186,139,199)',
      timeVisible: true,
      secondsVisible: true
    },
    localization: {
  ...
  function renderSeries(data) {
    ...
    data.series.applyOptions({
      priceLineWidth: 2,
      //lineSeries
      topColor: 'rgba(86,139,199, 0.8)',
      bottomColor: 'rgba(86,139,199, 0)',
      lineColor: 'rgba(21, 146, 230, 1)',
      //candleSeries
      upColor: '#6fa22f',
      downColor: '#ff001e',
      borderVisible: false,
      borderUpColor: '#6fa22f',
      borderDownColor: '#ff001e',
      wickUpColor: "#6fa22f",
      wickDownColor: "#ff001e",
    });
  }
  ...
  function updatePriceSummary(json) {
    ...
    market.forEach(i => {
      ...
      row.children[3].children[0].style.color = json.price[i].ZF === '0' ? '' : json.price[i].ZF === '1' ? '#ff001e' : '#6fa22f'
    })
    ...
    priceSummary[2].querySelector('span').style.color = json.price[data.currentMarket].ZF === '0' ? '' : json.price[data.currentMarket].ZF === '1' ? '#ff001e' : '#6fa22f'
          ...
          <div class="expand_btn">
            <a href="">
              <div class="tr">
                ...
                <div class="td ${json.order_all[i].ostyle === '0' ? 'td_price_Up' : 'td_price_Down'}">
  ...
  function sendOrder() {
    ...
    document
        ...
        .addEventListener('click', e => {
          ...
          axios({
            method: 'get',
            url: 'https://zb.objkss.cc/index.php?r=web/webapi/add',
            params: {
              pid: orderData.pid,
              uid: window.uid, //window.uid
              order_status: orderData.order_status,
              order_sec: orderData.order_sec,
              order_prc: orderData.order_prc,
              now_price: orderData.now_price
            }
          }).then(res => {
            if (res.data.type == 0) {
              swal({
  ...
  document.querySelector('.controlButtons_chartChangePanel').addEventListener('click', e => {
    ...
    if (data.currentSeries === 'lineSeries') {
      if (!document.querySelector('[data-period]')) {
        li = document.createElement('li')
        li.innerHTML = `<a href="javascript:;" data-period='1S'>1S</a>`
        document.querySelector('.dropdown-menu').prepend(li)
      }
    } else {
      if (document.querySelector('[data-period]')) {
        document.querySelector('[data-period]').remove()
      }

      if (data.activeInterval === '1S') {
        data.activeInterval = '1M'
        document.querySelector('.dropdown-toggle span').textContent = data.activeInterval
      }
    }
