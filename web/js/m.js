...
function autoHeight(){
	...
	$("footer").height(40+WinHss-headerbarH-headerH-NavH-tradebarH-ecBarH);
...
function getMaindata(ctype){
	...
	$.ajax({
		...
		success: function(_jdatadata) {
			...
			ccout=setTimeout("getonedata()",xunhuantime);
			getonedata();
...
function gotoecharts(data){
	...
	$("#container .txt2 span.c i").text(macd[macdL].toFixed(4));
	 ...
	 console.log(data0);
	  var ecKdata = {
		  legend: {
				//data: ['K线1M', 'K线5M', 'K线15M', 'K线30M', 'K线1H', 'K线1D']
			},
			tooltip : {
				trigger: 'axis',
				formatter: function (params) {
					...
					if(params[0].seriesName==undefined){
						params[0].seriesName="K线";
					}
					if(params[0].name==undefined){
						params[0].name="00:00";
					}
					var res = params[0].seriesName + ' ' + params[0].name;
					if(params[0].seriesName!="MACD"){
						res += '<br/>  今开 : ' + params[0].value[0] + '  最高 : ' + params[0].value[3];
