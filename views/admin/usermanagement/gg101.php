                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="bootstrap-data-table-export"
                                            ...
                                            for($i=0;$i<count($db_userinfo_down);$i++)
                                            {
                                                echo "<tr role=\"row\" class=\"odd\">";
                                                echo "<td data-th=\"用户ID\">".$db_userinfo_down[$i]->uid."</td>";
                                                echo "<td data-th=\"客户信息\">".$db_userinfo_down[$i]->username."</td>";
                                                echo "<td data-th=\"客户名称\">".$db_userinfo_down[$i]->nickname."</td>";


                                                echo "<td data-th=\"创建日期\">".date("Y-m-d H:i:s",$db_userinfo_down[$i]->utime)."</td>";


                                                echo "<td data-th=\"最近登陆\">".date("Y-m-d H:i:s",$db_userinfo_down[$i]->lastlog)."</td>";
                                                $ordertemp=0;
                                                for($j=0;$j<count($db_order);$j++){
                                                    if($db_userinfo_down[$i]->uid == $db_order[$j]->uid){
                                                        $ordertemp=$ordertemp+1;
                                                    }
                                                }
                                                echo "<td data-th=\"订单数\">$ordertemp</td>";
                                                echo "<td data-th=\"帐户馀额\" class=\"text-danger\">".$db_userinfo_down[$i]->usermoney."</td>";
