...
<body>
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content mt-3">
            ...
            <?php
            echo "<a href=". yii\helpers\Url::to(['admin/ordermanagement/orderrefresh'])."><button type=\"button\" class=\"btn btn-success\">点此刷新</button></a>";
            ?>
                                            ...
                                            <?php
                                                ...
                                                for($i=0;$i<count($db_order);$i++){
                                                    if($db_order[$i]->is_win == 1){  //贏的抓出來打
                                                        $order_temp=0-$db_order[$i]->ploss;
//                                                      echo $order_temp[$i]->uid."$order_temp"."<br>";
                                                    }
                                                    if($db_order[$i]->is_win == 2){ //輸的扣到他剩下本金
                                                        $order_temp=$db_order[$i]->ploss;
//                                                       echo $order_temp[$i]->uid."$order_temp"."<br>";
                                                    }
                                                    $temp=$order_temp+$temp;
                                                }
                                                echo "<div class=\"h5 mb-0 p-3 text-center\">$temp</div>";
                                            ?>
                                        </div>
                                    </div>
                                </div>
...
//                                                      echo "<td data-th=\"交易帐号\">".$db_order[$i]->orderno."</td>";
                                                      echo "<td data-th=\"用户姓名\">".$db_order[$i]->username."</td>";
                                                      echo "<td data-th=\"订单时间\">".date('Y-m-d H:i:s', $db_order[$i]->buytime)."</td>";
                                                      echo "<td data-th=\"产品信息\">".$db_order[$i]->ptitle."</td>";
                                                        if($db_order[$i]->ostaus == 0 ){
                                                            echo "<td data-th=\"订单状态\">交易中</td>";
                                                        }elseif($db_order[$i]->ostaus==1){
                                                            echo "<td data-th=\"订单状态\">平倉</td>";
                                                        }
                                                      if($db_order[$i]->ostyle == 0 ){
                                                          echo "<td data-th=\"方向\" class=\"text-danger\">买涨</td>";
                                                      }elseif($db_order[$i]->ostyle==1){
                                                          echo "<td data-th=\"方向\" class=\"text-success\">买跌</td>";
                                                      }
                                                      echo "<td data-th=\"时间/点数\">".$db_order[$i]->endprofit."</td>";
                                                      echo "<td data-th=\"建仓点数\">".$db_order[$i]->buyprice."</td>";
                                                      echo "<td data-th=\"平仓点数\" class=\"text-danger\">".$db_order[$i]->sellprice."</td>";
                                                      echo "<td data-th=\"委托金额\" >".$db_order[$i]->fee."</td>";
...
//                                                      echo "<td data-th=\"有效委托馀额\" class=\"text-danger\"></td>";
                                                      if($db_order[$i]->ostaus == 0 ){
                                                          ...
                                                          if($db_order[$i]->ostyle == 0 ){
                                                              echo "<td data-th=\"实际盈亏\" class=\"text-danger\">".($db_order[$i]->fee)*($db_order[$i]->endloss/100)."</td>";
                                                          }elseif($db_order[$i]->ostyle==1){
                                                              echo "<td data-th=\"实际盈亏\" class=\"text-success\">".($db_order[$i]->fee)*($db_order[$i]->endloss/100)."</td>";
                                                          }
                                                      }
                                                      if($db_order[$i]->ostaus == 0 ){
                                                          echo "<td data-th=\"手续费\"></td>";
                                                      }else{
                                                          echo "<td data-th=\"手续费\">".$db_order[$i]->sx_fee."</td>";
                                                      }

                                                      // if($db_order[$i]->is_win == 1){
                                                      //     echo "<td data-th=\"买后盈亏\" class=\"text-danger\">".$db_order[$i]->fee."</td>";
                                                      // }elseif($db_order[$i]->is_win == 2){
